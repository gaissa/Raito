...
$(document).ready(function() {
    $('form#change-username').validate({
        ...
        messages: {
            s_username: {
                required: '<?php echo osc_esc_js(__("Username: this field is required", raito_teema_THEME_FOLDER)); ?>.'
            }
        },
            ...
            cInterval = setInterval(function(){
                $.getJSON(
                    ...
                    function(data){
                        clearInterval(cInterval);
                        if(data.exists==0) {
                            $("#available").text('<?php echo osc_esc_js(__("The username is available", raito_teema_THEME_FOLDER)); ?>');
                        } else {
                            $("#available").text('<?php echo osc_esc_js(__("The username is NOT available", raito_teema_THEME_FOLDER)); ?>');
                        }
                    }
                );
            }, 1000);
...
<div class="row">
  ...
  <div class="title">
    <h1>
      <?php _e('Change Username', raito_teema_THEME_FOLDER); ?>
    </h1>
    ...
    <form action="<?php echo osc_base_url(true); ?>" method="post" id="change-username">
      ...
      <div class="form-group">
        <label class="control-label" for="s_username">
          <?php _e('Username', raito_teema_THEME_FOLDER); ?>
        </label>
        <div class="controls">
