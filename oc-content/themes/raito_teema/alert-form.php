...
$(document).ready(function(){
    ...
    $(".sub_button").click(function(){
        $.post('<?php echo osc_base_url(true); ?>', {email:$("#alert_email").val(), userid:$("#alert_userId").val(), alert:$("#alert").val(), page:"ajax", action:"alerts"},
            function(data){
                if(data==1) { alert('<?php echo osc_esc_js(__('You have sucessfully subscribed to the alert', raito_teema_THEME_FOLDER)); ?>'); }
                else if(data==-1) { alert('<?php echo osc_esc_js(__('Invalid email address', raito_teema_THEME_FOLDER)); ?>'); }
                else { alert('<?php echo osc_esc_js(__('Invalid email address', raito_teema_THEME_FOLDER)); ?>');
                };
        });
        return false;
    });
