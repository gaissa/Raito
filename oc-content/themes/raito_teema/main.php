<?php
    ...
    raito_teema_add_body_class('home');
...
<?php
    ...
    osc_get_premiums(raito_teema_premium_listings_shown_home());
...
<h1 class="title">
  <?php _e('Premium Listings',raito_teema_THEME_FOLDER);?>
</h1>
    ...
    <li class="listing-card premium col-md-3">
      <article class="loop_premium">
        ...
        <div class="listing-attr">
          ...
          <article> <span class="category"><i class="fa fa-<?php echo raito_teema_category_icon( osc_premium_category_id() ); ?>"></i><?php echo osc_premium_category() ; ?></span> <span class="location"><i class="fa fa-map-marker"></i><?php echo osc_premium_city(); ?>
          ...
          </a> <span>|</span> <a class="delete" onclick="javascript:return confirm('<?php echo osc_esc_js(__('This action can not be undone. Are you sure you want to continue?', raito_teema_THEME_FOLDER)); ?>')" href="<?php echo osc_premium_delete_url();?>" >
...
<div class="content">
  ...
  <div class="latest_ads">
    ...
    <p class="empty">
      <?php _e("There aren't listings available at this moment", raito_teema_THEME_FOLDER); ?>
    </p>
    <?php } else { ?>
    <?php
    ...
    <p class="see_more_link"><a href="<?php echo osc_search_show_all_url() ; ?>"> <strong>
      <?php _e('See all listings', raito_teema_THEME_FOLDER) ; ?>
      &raquo;</strong></a> </p>
    <?php } ?>
    <?php } ?>
  </div>
  <?php if(osc_get_preference('show_popular', 'raito_teema_theme') == '1'){?>
  <div id="tab_filter">
    <h2 class="title"> <?php echo sprintf(__('Popular in %s', raito_teema_THEME_FOLDER), osc_page_title()) ; ?> </h2>
    <?php if(raito_teema_show_popular_searches() ){ ?>
    <section id='Searches'>
      <div class="popular_cities">
        <?php $searches	=	raito_teema_popular_searches( raito_teema_popular_searches_limit() ); ?>
    ...
    <?php } ?>
    <?php if(raito_teema_show_popular_regions() ){ ?>
    <section id='Regions'>
      <div class="popular_regions">
        <?php $regions	=	raito_teema_popular_regions(raito_teema_popular_regions_limit()); ?>
    ...
    <?php } ?>
    <?php if(raito_teema_show_popular_cities() ){ ?>
    <section id='Cities'>
      <div class="popular_cities">
        <?php $cities	=	raito_teema_popular_cities(raito_teema_popular_cities_limit()); ?>
        <?php if(!empty($cities)){ ?>
        <ul>
  ...
  <?php } ?>
  <?php if( osc_get_preference('homepage-728x90', 'raito_teema_theme') != "") { ?>
  <div class="ads_home"> <?php echo osc_get_preference('homepage-728x90', 'raito_teema_theme'); ?> </div>
  <?php } ?>
</div>
