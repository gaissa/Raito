...
<h1 class="title"><?php _e('Categories', raito_teema_THEME_FOLDER);?></h1>
<div class="row">
<?php
...
<ul class="col-sm-6 col-md-3 grid_list">
  <li>
    <section class="listings">
     <h2><i class="fa fa-<?php echo raito_teema_category_icon( osc_category_id() ); ?>"></i>
      <?php
    ...
    <ul>
      <?php while ( osc_has_subcategories() ) { if( $m<=(osc_get_preference('sub_cat_limit', 'raito_teema_theme'))){?>
      <li>
      ...
      <?php } $m++; } if($m>(osc_get_preference('sub_cat_limit', 'raito_teema_theme'))+1) {?>
      <li class="last"><a href="<?php echo $_url; ?>"><strong><?php _e('See more listings...', raito_teema_THEME_FOLDER);?></strong></a></li>
      <?php } ?>
    </ul>
    <?php } ?>
    </section>
...
<?php
	...
	if( !function_exists('delete_user_item_js') ) {
        function delete_user_item_js() {
            $location = Rewrite::newInstance()->get_location();
            $section  = Rewrite::newInstance()->get_section();
            if( ($location === 'user' && in_array($section, array('items'))) || (Params::getParam('page') ==='custom' && Params::getParam('in_user_menu')==true ) ) {
...
<script type="text/javascript">
    raito_teema.user = {};
    raito_teema.user.id = '<?php echo osc_user_id(); ?>';
    raito_teema.user.secret = '<?php echo osc_user_field("s_secret"); ?>';
</script>
<?php }
        }
		...
		function theme_raito_teema_actions_admin() {
			...
			switch( Params::getParam('action_specific') ) {
				...
				case('templates_home'):
					...
					osc_redirect_to(osc_admin_render_theme_url( 'oc-content/themes/'.raito_teema_THEME_FOLDER.'/admin/settings.php#templates' ));			break;
    ...
    function raito_teema_delete() {
        Preference::newInstance()->delete(array('s_section' => raito_teema_THEME_FOLDER));
    }
...
function raito_teema_print_sidebar_category_search($aCategories, $current_category = null, $i = 0)
{
    ...
    if(!isset($c['pk_i_id'])) {
        echo '<ul '.$class.'>';
        if($i==1) {
            echo '<li><a href="'.osc_esc_html(osc_update_search_url(array('sCategory'=>null, 'iPage'=>null))).'">'.__('All categories', raito_teema_THEME_FOLDER)."</a></li>";
        }
...
<ul <?php echo $class;?>>
  ...
  <li><a href="<?php echo osc_esc_html(osc_update_search_url(array('sCategory'=>null, 'iPage'=>null))); ?>">
    <?php _e('All categories', raito_teema_THEME_FOLDER); ?>
    </a></li>
  <?php } ?>
  <li> <a id="cat_<?php echo osc_esc_html($c['pk_i_id']);?>" href="<?php echo osc_esc_html(osc_update_search_url(array('sCategory'=> $c['pk_i_id'], 'iPage'=>null))); ?>">
    ...
    <?php raito_teema_print_sidebar_category_search($aCategories, $current_category, $i); ?>
  </li>
  <?php if($i==1) { ?>
  <?php } ?>
</ul>
	...
	$('form[name=item]').validate({
		rules: {
			...
			'title[<?php echo osc_current_user_locale();?>]': {
				required:true,
				minlength:<?php echo raito_teema_title_minimum_length();?>
			},
			'description[<?php echo osc_current_user_locale();?>]': {
				minlength:<?php echo raito_teema_description_minimum_length();?>
			},
			price: {
				maxlength: 50
			},
			...
			countryId:{
				required: <?php echo raito_teema_locations_required(); ?>
			},
			region: {
				required: <?php echo raito_teema_locations_required(); ?>,
				minlength: 3,
			...
			<?php if(raito_teema_locations_input_as()=='select'){ ?>
			,
			regionId: {
				required: <?php echo raito_teema_locations_required(); ?>
			},
		...
		messages: {
			catId: {
			required: "<?php echo osc_esc_js(__("Choose one category", raito_teema_THEME_FOLDER)); ?>."
			},
			'title[<?php echo osc_current_user_locale();?>]': {
				required: "<?php echo osc_esc_js(__("Title: this field is required", raito_teema_THEME_FOLDER)); ?>.",
				minlength: "<?php echo osc_esc_js(__("Title too short", raito_teema_THEME_FOLDER)); ?>."
			},
			'description[<?php echo osc_current_user_locale();?>]': {
				minlength: "<?php echo osc_esc_js(__("Description too short", raito_teema_THEME_FOLDER)); ?>."
			},
			price: {
				maxlength: "<?php echo osc_esc_js(__("Price: no more than 50 characters", raito_teema_THEME_FOLDER)); ?>."
			},
			currency: "<?php echo osc_esc_js(__("Currency: make your selection", raito_teema_THEME_FOLDER)); ?>.",
			"photos[]": {
				accept: "<?php echo osc_esc_js(__("Photo: must be png,gif,jpg,jpeg", raito_teema_THEME_FOLDER)); ?>."
			},
			contactName: {
				required: "<?php echo osc_esc_js(__("Name: this field is required", raito_teema_THEME_FOLDER)); ?>.",
				minlength: "<?php echo osc_esc_js(__("Name: enter at least 3 characters", raito_teema_THEME_FOLDER)); ?>.",
				maxlength: "<?php echo osc_esc_js(__("Name: no more than 35 characters", raito_teema_THEME_FOLDER)); ?>."
			},
			contactEmail: {
				required: "<?php echo osc_esc_js(__("Email: this field is required", raito_teema_THEME_FOLDER)); ?>.",
				email: "<?php echo osc_esc_js(__("Invalid email address", raito_teema_THEME_FOLDER)); ?>."
			},
			countryId: {
				required: "<?php echo osc_esc_js(__("Please select a country", raito_teema_THEME_FOLDER)); ?>."
			},
			region: {
				required: "<?php echo osc_esc_js(__("Region: this field is required", raito_teema_THEME_FOLDER)); ?>.",
				minlength: "<?php echo osc_esc_js(__("Region: enter at least 3 characters", raito_teema_THEME_FOLDER)); ?>.",
				maxlength: "<?php echo osc_esc_js(__("Region: no more than 100 characters", raito_teema_THEME_FOLDER)); ?>."
			},
			city: {
				required: "<?php echo osc_esc_js(__("City: this field is required", raito_teema_THEME_FOLDER)); ?>.",
				minlength: "<?php echo osc_esc_js(__("City: enter at least 3 characters", raito_teema_THEME_FOLDER)); ?>.",
				maxlength: "<?php echo osc_esc_js(__("City: no more than 100 characters", raito_teema_THEME_FOLDER)); ?>."
			}
			<?php if(raito_teema_locations_input_as()=='select'){ ?>
			,
			regionId: {
