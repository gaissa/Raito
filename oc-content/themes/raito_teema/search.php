    ...
    $(function() {
                    ...
                    $( "#sCity" ).autocomplete({
                        ...
                        select: function( event, ui ) {
                            ...
                            log( ui.item ?
                                "<?php echo osc_esc_html( __('Selected', raito_teema_THEME_FOLDER) ); ?>: " + ui.item.value + " aka " + ui.item.id :
                                "<?php echo osc_esc_html( __('Nothing selected, input was', raito_teema_THEME_FOLDER) ); ?> " + this.value );
                        }
                    });
                });
    </script>
...
<div class="row">
  ...
  <div class="col-sm-8 col-md-9">
    ...
    <div class="toolbar">
      ...
      <p class="empty" ><?php printf(__('There are no results matching "%s"', raito_teema_THEME_FOLDER), osc_search_pattern()) ; ?></p>
      <?php } else { ?>
      <span class="counter-search">
      <?php
                $search_number = raito_teema_search_number();
                printf(__('%1$d - %2$d of %3$d listings', raito_teema_THEME_FOLDER), $search_number['from'], $search_number['to'], $search_number['of']);
            ?>
      </span>
    ...
    <?php } ?>
    <?php if(osc_count_items() > 0) {
        #echo '<h5 class="title titles">'.__('Listings',raito_teema_THEME_FOLDER).'</h5>';
		?>
    <?php 
    ...
    <?php } ?>
    <?php if( osc_get_preference('search-results-middle-728x90', 'raito_teema_theme') != "" ){ ?>
    <div class="ads_search_bottom"> <?php echo osc_get_preference('search-results-middle-728x90', 'raito_teema_theme'); ?></div>
    <?php } ?>
  </div>
</div>
